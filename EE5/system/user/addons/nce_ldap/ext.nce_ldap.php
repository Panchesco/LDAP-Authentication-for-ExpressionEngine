...
// ----------------------
	...
	*/
	function update_extension($current = '')
	{
		...
		ee()->db->query('UPDATE exp_extensions SET version = \''.ee()->db->escape_str($this->version).'\' WHERE class = \''.ee()->db->escape_str(__CLASS__).'\'');
	}
...
// ----------------------
	...
	*/
	...
	{
		ee()->db->query('DELETE FROM exp_extensions WHERE class = \''.ee()->db->escape_str(__CLASS__).'\'');
	}
...
// ----------------------
	...
	{
		$provided_username = ee()->input->post('username');
		$provided_password = ee()->input->post('password');
...
// ----------------------
	...
	function sync_user_details($user_info)
	{
			...
			//$encrypted_password = ee()->functions->hash(stripslashes($user_info['password']));
			
			$encrypted_password  = hash('sha1',stripslashes($user_info['password']));
			
			$sql = 'UPDATE exp_members SET password = \''.ee()->db->escape_str($encrypted_password).'\' WHERE username = \''.ee()->db->escape_str($user_info['username']).'\'';
...
// ----------------------
	...
	{
		$sql = 'SELECT \'username\' FROM exp_members WHERE username = \''.ee()->db->escape_str($user_info['username']).'\'';
		...
		if ($query->num_rows === 0)
		{
			...
			$member_id = ee()->member_model->create_member($data);
			if ($member_id > 0) // update other relevant fields
			{
				...
				//ee()->db->query('INSERT INTO exp_member_data SET member_id = '.ee()->db->escape_str($member_id));
				//ee()->db->query('INSERT INTO exp_member_homepage SET member_id = '.ee()->db->escape_str($member_id));
				...
				ee()->session->userdata['ldap_message'] = $this->settings['first_time_login_message'];
			}
			else
			{
...
// ----------------------
	...
	function authenticate_user($conn, $username, $password, $ldap_username_attribute, $ldap_search_base)
	{
		...
		if ($result === FALSE)
		{
			ee()->session->userdata['ldap_message'] = $this->settings['no_ldap_login_message'];
